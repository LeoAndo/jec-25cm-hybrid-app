...
- PR本文は「概要／変更内容／判断したこと／検証／実績」の順に書き、`Closes #<番号>` を入れる。検証の節に「※ リポジトリの方針により、Unit Test は対象外です。」と書く。
  ...
  - **PR本文は、Draftのうちに完成版まで仕上げる。** レビューbotが動き出す契機はpushではなくDraftの解除なので、Draftのあいだは本文が書き戻されない。仕上げたら `gh pr view <番号> --json body` で読み直し、書いた内容が残っていることを確かめてから `gh pr ready` する。
...
- 自動レビューの指摘は、そのまま実行しない。現在のソースと検査結果で再確認してから判断する。**チェックが `SUCCESS` でも、そのbotがレビューしたとは限らない。** 上限やトライアル終了で未実施のまま `SUCCESS` になるbotがある。
  ...
  - **このリポジトリでは、どのbotがどう動くかをまだ実測していない。** 最初にDraftを外したPRで、実際に投稿されたレビューと、チェックの状態・所要時間・本文が書き戻されたかどうかを記録し、bot名・Draft中の挙動・Draft解除後の挙動・指摘が出るかの表をこの節に書き足すissueを立てる（§6）。実測せずに、ほかのリポジトリの表を写さない。
...
1. **素のHTML / CSS / JavaScript で書く。フレームワークもビルドツールも入れない。** 受講生はWeb基礎で素のHTML/CSS/JSを書いている。ここに Vue / React / webpack を挟むと、「自分が書いたコードが画面のどこになるか」が1行で追えなくなる。
...
- **`true` に上げてよいのは、次の2つが両方終わった言語だけ。**
  ...
  2. 翻訳したのとは別のAIが、新しく訳した文を日本語へ逆翻訳し、原文と照合し終えている。**翻訳と照合は別のPR・別のセッションで行う。** 自分で訳して自分で照合しない。
