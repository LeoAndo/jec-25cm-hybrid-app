...
- **アラビア語とモンゴル語は、まだ `config/i18n.json` に足さない。** アラビア語は右から左に書く言語で、いまの生成スクリプトとCSSでは表示が崩れる（[issue #39](https://github.com/LeoAndo/jec-25cm-hybrid-app/issues/39)）。モンゴル語は、キリル文字か伝統的モンゴル文字（縦書き）かを学生本人に確かめてから決める（[issue #40](https://github.com/LeoAndo/jec-25cm-hybrid-app/issues/40)）。
