...
2026-09-25、PR #65（#46）で変えた`www/css/style.css`（一覧の行の右の余白30px）を公開用のMonacaプロジェクトへ保存し直し、`index.html`・`style.css`・`app.js`・`config.xml`・`package.json`のエディター上の全文が`main`（332011a）と一致することをSHA-256で照合しました（issue #94）。公開URLは作り直していません。公開URLからの取り込みは、保存した最新の内容を取り込ませます。同じしくみの確認として、同日にM02の公開URLから空きの1枠へ取り込んだコピーで、保存し直した`style.css`・`app.js`が`main`と一致することを確かめました（`M02OshiSave/README.md`）。M02のコピーのプレビューでは、60文字の名前が右端の目印と重ならないことも測って確かめています（M01と同じCSS）。M01の公開URLからの取り込みは、採点用の枠を空けておくため行っていません。
...
[完成見本をMonacaへ取り込む](https://monaca.mobi/ja/directimport?pid=6ab62c99e7888517430a0ae6)。このURLはアプリのWeb公開ではなく、プロジェクトの取り込み用です。取り込む前に3枠の空きを確認してください。このURLは2026-09-25に取り込み直したプロジェクトで発行し直したものです（上記）。
