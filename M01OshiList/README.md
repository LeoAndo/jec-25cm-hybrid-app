# M01OshiList — 推しログの一覧と詳細

Monacaのコマ1〜4で作る「推しログ」の完成プロジェクトです。学びや開発の中で見つけた「NotebookLM」「Android Studio / Xcode」「AIエージェント開発」の3件を推しカードにし、押した項目の推しポイントや次に試したいことを読みます。授業では静的なカードから順に作り、この完成版は動作確認とコード比較に使います。

## 画面の構成

| 画面 | 表示するもの | 操作 |
| --- | --- | --- |
| 一覧 | 推しの名前・ジャンルを載せた3枚のカード、選択した項目 | カードを押すと、その推しの詳細を開く |
| 詳細 | 名前、ジャンル、私の推しポイント・次に試したいこと | 左上の「戻る」で一覧へ戻る |
...
| `config.xml` | アプリ名「推しログ」、ID `jp.ac.jec.m01oshilist` |
...
4. 各行の名前とジャンルを`textContent`で設定し、クリックイベントを登録します。
5. 行を押すと選択名を更新し、`pushPage`の`data`で項目を詳細ページへ渡します。
6. 詳細の`init`で、`page.data.oshi`から名前・ジャンル・コメントを表示します。
...
- `ons-list-item`に`oshi-card`を付け、余白と角丸の枠でカードにします。`width: auto`で左右の余白を含めて画面に収め、一覧と行の既定罫線を外します。ツールバー・戻る部品・押したときの表現はOnsen UIに任せます。
...
| `style.css`の`.app-caption`・`.app-selection`の`color` | 詳細のジャンル・一覧上部の説明と、選択名の色 |
...
Google Chromeで[ローカルの推しログ](http://127.0.0.1:8765/)を開きます。停止するときはターミナルで`Control + C`を押します。`index.html`を`file://`で直接開かず、HTTPで確認します。
...
- 一覧の3項目をそれぞれ開くと、対応する名前・ジャンル・コメントが表示される。
...
今回、3件の題材と画面の文言を「推しを集めて読む」内容にそろえ、一覧をカード表示にしました。データの3プロパティと画面遷移の処理は変更していません。変更後もローカルChromeの一覧・詳細と、公開用プロジェクトのiPhone 15／Pixel 8プレビューのカード表示・詳細・戻る・選択保持を確認しました。Pixel 8では連続クリックで詳細が1画面のままであることも確認しています。以下の4件目・HTML記号の確認は、処理が同じ変更前の検証用コピーで行ったものです。
...
[完成見本をMonacaへ取り込む](https://monaca.mobi/ja/directimport?pid=6ab3e25de78885da2c0a0ae6)。このURLはアプリのWeb公開ではなく、プロジェクトの取り込み用です。取り込む前に3枠の空きを確認してください。公開URLからの再取り込みは未確認です。
