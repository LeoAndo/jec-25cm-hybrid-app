...
同日、公開用のMonacaプロジェクトへ同じ`index.html`・`style.css`・`app.js`・`config.xml`・`package.json`を保存し、エディター上の全文がこのフォルダと一致することをSHA-256で照合しました。iPhone 15とPixel 8のプレビューでは、Reload後に全3件の詳細・理由の改行・戻る・選択名の保持を確認しました。行を3回続けて押しても詳細は1画面だけで、戻る1回で一覧へ戻ることも確認しました。公開URLは作り直していません。保存した内容が公開URLから取り込まれることはM02で確認しましたが、M01の公開URLからの再取り込みは行っていません。
